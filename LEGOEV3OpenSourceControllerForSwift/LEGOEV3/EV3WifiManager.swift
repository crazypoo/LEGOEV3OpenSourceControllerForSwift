...
public class EV3WifiManager: NSObject {
    ...
    func udpSocket(sock:GCDAsyncUdpSocket,data:NSData,address:NSData,filterContext:AnyObject)
    {
        ...
        if (msg != nil)
        {
            ...
            if host?.length < 20 && device == nil {
                let aDevice = EV3Device.init(serialNumber: String(serialNumber), address: String(host), tag: UInt(devices!.count), isConnected:false)
                ...
                devices?.setObject(aDevice, forKey: aDevice.address)
                if (delegate != nil) {
                    delegate?.updateView()
                }
            }
    ...
    {
        let tcpSocket:GCDAsyncSocket = device.tcpSocket
        do
        {
            try tcpSocket.connectToHost(device.address, onPort: 5555)

            let unlockMsg = "GET /target?sn=\(device.serialNumber) VMTP1.0 Protocol: EV3"
    ...
    func disconnectTCPSocketWithDevice(device:EV3Device)
    {
        ...
        devices?.removeObjectForKey(device.address)
        if (delegate != nil) {
    ...
    func socket(sock:GCDAsyncSocket,data:NSData,tag:Int64)
    {
        ...
        device.handleReceivedData(data, withTag: Int(tag))
    }
